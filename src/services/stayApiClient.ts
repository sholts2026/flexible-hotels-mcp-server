...
export class StayApiClient {
  ...
  async searchHotels(params: {
            ...
            const data = await this.get<{ success: boolean; data?: unknown; message?: string }>("/booking/search", {
          ...
          // StayAPI's documented shape has `data` as an array of hotel results, but in
          // practice (observed live) it can come back as a non-array value instead.
          // DIAGNOSTIC: surface a SAFE, structured description of the shape (never the raw
          // payload itself, which can trip content filters if it embeds URLs/tokens) via a
          // thrown (caught upstream, non-fatal) error, so we can see what StayAPI returned
          // without dumping raw values.
          if (!Array.isArray(data.data)) {
                      const dataType =
                                    data.data === null ? "null" : data.data === undefined ? "undefined" : typeof data.data;
                      const dataKeys =
                                    data.data && typeof data.data === "object" ? Object.keys(data.data as object).join(",") : "n/a";
                      const topKeys = Object.keys(data as object).join(",");
                      throw new StayApiError(
                                    `Unexpected /booking/search response shape: data.data type=${dataType} keys=[${dataKeys}] ` +
                                      `topLevelKeys=[${topKeys}] success=${data.success} messageLength=${(data.message ?? "").length}`,
                                  );
          }
