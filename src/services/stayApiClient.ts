...
export class StayApiClient {
  ...
  async searchHotels(params: {
        ...
        // DIAGNOSTIC: surface the raw shape via a thrown (caught upstream, non-fatal)
        // error instead of silently treating it as zero results, so we can see exactly
        // what StayAPI actually returned for a real query.
        if (!Array.isArray(data.data)) {
                  throw new StayApiError(
                              `Unexpected /booking/search response shape (data.data is not an array). Raw response: ${JSON.stringify(data).slice(0, 800)}`,
                            );
        }
