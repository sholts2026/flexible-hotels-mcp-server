...
export class StayApiClient {
  ...
  async searchHotels(params: {
              ...
              // DIAGNOSTIC (2026-08-25): individual hotel objects came back with a
              // field-name mismatch too (min_total_price guess was wrong, every hotel
              // got filtered out downstream as "no price"). Surface the SAFE key list
              // of the first hotel object (never raw values) so the real field names
              // can be confirmed, same approach used to find `data.hotels` above.
              if (results.length > 0) {
                              const first = results[0] as Record<string, unknown>;
                              const keys = Object.keys(first).join(",");
                              throw new StayApiError(`DIAGNOSTIC hotel object keys=[${keys}]`);
              }
